...
/*{
  ...
  if (fp && info && res && res->mem && res->d) {
    ...
    const char *Delim = is_dump ? (const char *)"" : delim; 
    ...
    int resdate, restime;
    int *width = NULL;
    int ndigits = log10(MAX(1,info->npools)) + 1; 
    ...
    if (is_odbtool) {
      ...
      int nra = res->nra;
      uint *dtnum = NULL;
      int len_ODB_tag_delim = STRLEN(ODB_tag_delim); 
      ...
      konvert = ODBc_conv2degrees(res, true); 
    ...
    Delim = is_dump ? (const char *)"" : delim; 
    for (j=0; j<ncols; j++) {
      col_t *colthis = &info->c[j];
      if (colthis->kind == 1 || colthis->kind == 2 || colthis->kind == 4) {
	const char *s = colthis->nickname ? colthis->nickname : colthis->name;
...
/*{
  ...
  if (first_time) {
    ...
    if (reopen_per_view && !reopen_per_pool) {
      ...
      FCLOSE_FP_OUT();
      ALLOC(ppfile,pplen);
      snprintf(ppfile,pplen,outfile,view); 
      ODB_fprintf(fp_echo, "Opening file '%s'\n",ppfile);
